...
import jnit.cms.sync.JnitSyncService;
...
public class CmsController {
    ...
    public String actionLogin(
    	...
    	if(resVO.getExpireDate() != null) {
    		...
    		if(today.isAfter(expire)) {
    			model.addAttribute("alert", "만료된 회원입니다.");
        ...
        if (resVO != null && resVO.getMbrId() != null && !resVO.getMbrId().equals("")) {
        	...
        	if(isLock == 0) {
        		...
        		/*
        		//exts 패키지 관리자회원정보 따로처리.
        		ComMbrVO admVO = new ComMbrVO();
        		BeanUtils.copyProperties(resVO, admVO);
        		sessReturnUrl = comMbrService.login(admVO);
        		*/
    ...
    public String searchPwFind(
		...
		String contextPath = Globals.CONTEXT_PATH.toString();
