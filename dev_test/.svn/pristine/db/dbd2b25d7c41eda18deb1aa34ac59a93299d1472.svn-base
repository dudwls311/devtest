			...
			if(listFile.length > 0){
				for(int i = 0 ; i < listFile.length ; i++){
					if(listFile[i].isFile()){
						listFile[i].delete(); 
						JnitFilegoUtil.deleteFile(JnitFilegoFileType.CONTENT, listFile[i]);		//Filego 삭제
					}else{
				...
				JnitFilegoUtil.deleteFile(JnitFilegoFileType.CONTENT, deleteDirectory);		//Filego 삭제
			}
		} catch (NullPointerException e){
			log.error(e.getMessage());
		}catch(Exception e){
