...
public class JnitboardLatestController {
	...
	throws Exception {
		...
		String latestDirPath = Globals.CONTEXT_PATH + SEPERATOR + BOARD_LATEST_PATH;
		...
		if(equalData){
			...
			latestFileW.flush();
			latestFileW.close();
			JnitFilegoUtil.transferFile(JnitFilegoFileType.BOARD, new File(latestPath));		//Filego 배포
		}
	...
	private boolean latestBoardDataFile(String root, String filePath) throws Exception{
		...
		latestFileW.flush();
		latestFileW.close();
		JnitFilegoUtil.transferFile(JnitFilegoFileType.BOARD, new File(filePath));		//Filego 배포
		return isSuccess;
	}
}