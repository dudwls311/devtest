...
public class JnitboardfileServiceImpl extends EgovAbstractServiceImpl implements
    ...
    public static final String cryptoKey = EgovProperties.getProperty("Globals.fileCodeView.key");//암호화키
	 ...
	 * @return
	 */
	public String encrypt(JnitboardfileVO searchVO){
		//DB조회를 없애기 위해 파일경로를 암호화해서 전송
		String str = searchVO.getFilePath() + "|" + searchVO.getFileNm()  + "|" + searchVO.getFileOnm();
		try{
	...
	public JnitboardfileVO decrypt(String str)throws EgovBizException{
		...
		ret.setFilePath(EgovWebUtil.filePathBlackList(decrypts[0]));
		ret.setFileNm(EgovWebUtil.filePathBlackList(decrypts[1]));
		ret.setFileOnm(EgovWebUtil.filePathBlackList(decrypts[2]));
		return ret;
	}
