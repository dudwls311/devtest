...
public class visitUtil {
    ...
    public static JSONObject visitStats() throws ParseException, IOException{
		...
		String visitDirPath = Globals.CONTEXT_PATH + SEPERATOR + VISIT_PATH;
    	String latestDirPath = Globals.CONTEXT_PATH + SEPERATOR + VISIT_LATEST_PATH;
		...
		if(hasFile){	//파일이 존재하면
			...
			latestFileT.flush();
			latestFileT.close();
			JnitFilegoUtil.transferFile(JnitFilegoFileType.CONTENT, new File(latestPathT));		//Filego 배포
