...
public class JnitcmsmenuServiceImpl extends EgovAbstractServiceImpl implements
    ...
    public String insertJnitcmsmenu(JnitcmsmenuVO vo, String SitePath, HttpServletRequest request) throws Exception {
    	...
    	String makeDir = Globals.CONTEXT_PATH+ afterPath;
    	...
    	while(!mkFile){
    		...
    		if(af.exists()) {
        		makeDir = Globals.CONTEXT_PATH+ afterPath+"-"+i;
        		j = i;
        		i++;
        		fileChange = true;
        	}else{
	    	...
	    	if(vo.getDepth() > 0) {
	    		...
	    		for(int i=vo.getDepth()-1; i>=0; i--) {
	    			if(!pid.equals("")) {
		    			...
		    			pid = NullUtil.nullString(parentVO.getPid());
	...
	public String makeMenuRedirect(JnitcmsmenuVO vo) throws Exception {
...
public String makeMenuNavTitleNames(JnitcmsmenuVO vo, String siteNm, String sitePath) throws Exception {
	...
	public String makeMenuPid(JnitcmsmenuVO vo, String type) throws Exception {
		String pidList = "";
		if(vo.getDepth() > 0) {
			String pid = "";
			pid = vo.getPid();
