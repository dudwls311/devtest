...
<form id="listPageForm" action="?">
	...
	<input type="hidden"  name="positionId" value="<c:out value="${searchVO.positionId}" />" />
	<input type="hidden"  name="deptId" value="<c:out value="${searchVO.deptId}" />" />
	<input type="hidden"  name="manager" value="<c:out value="${searchVO.manager}" />" />
</form>
			<form action="?">
			...
			<div class="search_wrap">
				<div class="search_left">
					...
					<c:forEach var="plist" items="${pos}">
							<option value="${plist.positionId}"${searchVO.positionId == plist.positionId ?' selected="selected"':'' }>${plist.positionNm}</option>
					</c:forEach>
					</select>
					<select id="deptId" name="deptId">
							<option value="">부서전체</option>
					<c:forEach var="list" items="${dpt}">
							<option value="${list.deptId}"${searchVO.deptId == list.deptId ?' selected="selected"':'' }>${list.deptNm}</option>
					</c:forEach>
					</select>
				</div>
				<br/>
				<div class="search_right">
					...
					<label for="searchKeyword" class="comment">관리자 입력</label>
					<input type="text" id="manager" name="manager" value="<c:out value="${searchVO.manager}" />" class="st_input" placeholder="관리자를 입력하세요"/>
