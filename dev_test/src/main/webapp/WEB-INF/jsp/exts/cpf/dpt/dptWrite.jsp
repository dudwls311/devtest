...
<form id="viewPageForm" action="?">
	...
	<input type="hidden" name="searchKeyword" value="<c:out value="${param.searchKeyword }" />" />
	<input type="hidden" name="pageIndex" value="<c:out value="${param.pageIndex }" />" />
	<input type="hidden" name="cdMode" value="view" />
	<input type="hidden" name="deptId" value="<c:out value="${result.deptId }" />" />
</form>

<form id="writeForm" action="?" method="post" enctype="multipart/form-data">
	<input type="hidden" name="cdMode" value="writeActionJson" />
					...
					<table class="table_style table_t_left th_v_m">
						<colgroup>
							...
							<tr>
								<th scope="row">부서명</th>
								<td>
									<input type="text" class="text" style="width:180px;" id="deptNm" name="deptNm" value="<c:out value="${result.deptNm}" />" />
								</td>
							</tr>
