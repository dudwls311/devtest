...
$(document).ready(function(){
	...
	$("input[name=upfile]").on("change", function(){
		...
		size = (Math.round(size * 100) / 100) + '' + fsExt[i];
		$("#sizeSpan").empty().text('파일 사이즈:'+size);
	});
});
...
function fnSetItemSourt(){
	var groupOrderType = $("#groupId option:selected").data('groupordertype');
	if(groupOrderType != '' && groupOrderType != undefined){
		if(groupOrderType != '1'){
			$("#itemSort").prop('readonly', true);
			$("#itemSortTxt").show();
		}else{
			$("#itemSort").prop('readonly', false);
			$("#itemSortTxt").hide();
		}
	}
}
							...
							<table class="table table-bordered table-striped th_left" summary="아이템 테이블">
									...
									<td><%-- 
										<form:select path="groupId" id="groupId">
											...
											<c:forEach var="group" items="${groupList}">
												<form:option value="${group.groupId}" label="${group.groupNm}" data-acd="${group.groupOrderType }" ></form:option>
											</c:forEach>
										</form:select>
										 --%>
										<select id="groupId" name="groupId">
											...
											<c:forEach var="group" items="${groupList}">
												<option value="${group.groupId}" label="${group.groupNm}" data-groupordertype="${group.groupOrderType }" ${group.groupId == result.groupId ? 'selcted' : '' } ><c:out value="${group.groupNm}" /></option>
											</c:forEach>
										</select>
									</td>
								</tr>
								<tr>
									...
									<td>
										...
										<span id="itemSortTxt" style="display: none;">&nbsp;&nbsp;&nbsp;그룹의 정렬 방식이 기본일 경우 사용가능</span>
									</td>
								</tr>
