...
function fn_egov_copy() {
	...
	if(confirm('복사 하시겠습니까?')){
		if($("#sitePath").val().length>2){
			...
			frm.action = "<c:url value='/cms/siteCopy.do'/>";
			frm.submit();
		}else{
			alert("사이트 경로를 두글자 이상 입력해주시기 바랍니다.");
		}
           	...
           	<div class="span12">
               	<div class="widget-box">
                   	<div class="widget-title">
						...
						<c:if test="${param.copyYn != 'Y'}">
							<a href="javascript:fn_egov_save();" class="btn btn-small"><i class="icon-ok-sign"></i> 저장</a>
						</c:if>
						<c:if test="${registerFlag != '수정' && param.copyYn == 'Y'}">
							<a href="javascript:fn_egov_copy();" class="btn btn-small"><i class="icon-ok-sign"></i> 복사</a>
						</c:if>
						<c:if test="${registerFlag == '수정'}">
						...
						<form:form commandName="jnitcmssiteVO">
						    ...
						    <table class="table table-bordered table-striped th_left" summary="사이트 등록">
						        <tbody>
						        	<c:if test="${registerFlag != '수정' && param.copyYn == 'Y'}">
							        	<tr>
							                <th style="width: 120px;"><font color="red">*</font> 복사할 사이트&nbsp;<a href="javascript:Jnit_site_question('1');"><i class="icon-question-sign"></i></a></th>
							                <td>
								                <select name="originSiteId" id="originSiteId">
						            ...
						            <tr>
						                <th style="width: 120px;"><font color="red">*</font> 사이트명&nbsp;<a href="javascript:Jnit_site_question('1');"><i class="icon-question-sign"></i></a></th>
						                <td>
