...
$(document).ready(function(){
	...
	$("input[type=radio][name=reserveYn]").on("change", function(){
		fn_reserveYnChange();
	});
});

function fn_reserveYnChange(){
	var reserveYn = $("input[type=radio][name=reserveYn]:checked").val();
	if(reserveYn == 'Y'){
		$("#reserveDate").attr('disabled', false);
		$("#reserveDate").css('background-color', 'white');
	}else{
		$("#reserveDate").val('');
		$("#reserveDate").attr('disabled', true);
		$("#reserveDate").css('background-color', '');
	}
}
...
<form:form commandName="jnitcmsmenuVO"  onsubmit="return fn_egov_save();" enctype="multipart/form-data">
	...
	<div class="content_in"> 
		<table class="table table-bordered">				
			...
			<tr>
				<th id="reserveTr" rowspan="2" style="text-align:left;">예약설정</th>
				<td style="text-align:left;"><b>활성화 여부</b></td>
				<td>
					<input type="radio" id="reserveYnY" name="reserveYn" value="Y" <c:out value="${jnitcmsmenuVO.reserveYn == 'Y' ? 'checked' : ''}"/>><label for="reserveYnY">예</label>
					<input type="radio" id="reserveYnN" name="reserveYn" value="N" <c:out value="${jnitcmsmenuVO.reserveYn == 'N' ? 'checked' : ''}"/>><label for="reserveYnN">아니요</label>
				</td>
			</tr>
			<tr>
				<td><b>예약일 설정</b></td>
				<td>
					<fmt:formatDate var="reserveDate" value="${jnitcmsmenuVO.reserveDate}" pattern="yyyy-MM-dd HH:mm"/>
					<input type="text" id="reserveDate" name="reserveDate" value="<c:out value="${reserveDate}" />" ${jnitcmsmenuVO.reserveYn == 'N' ? 'disabled' : ''} readonly="readonly" />
				</td>        
			</tr>
