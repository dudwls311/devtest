...
DROP TABLE  IF EXISTS `jnitboardinfo_history`;
...
DROP TABLE IF EXISTS `jnit_com_smpl`;
DROP TABLE IF EXISTS `jnit_com_code`;
DROP TABLE IF EXISTS `jnit_com_code_grp`;
/**********************************/
...
CREATE TABLE JNITALLIM_GROUP(
		...
		GROUP_ORDER_TYPE                	VARCHAR(2)		 NULL  COMMENT '배너 정렬 유형'
) COMMENT='알리미 그룹 테이블';
...
/**********************************/
/* Table Name: 게시판 설정 이력 테이블 */
/**********************************/
CREATE TABLE JNITBOARDINFO_HISTORY (
	BOARD_ID varchar(20) NOT NULL,
	BOARD_GROUP varchar(100) NOT NULL,
...
ALTER TABLE JNITBOARDINFO_HISTORY ADD CONSTRAINT IDX_JNITBOARDINFO_HISTORY_PK1 PRIMARY KEY (BOARD_ID, MBR_ID, ACTION_DATE);
...
CREATE INDEX JNITBOARDFILE_IDX2 ON JNITBOARDFILE(BOARD_CNT_ID);
...
CREATE TABLE JNITCMSMENU(
		...
		RESERVE_YN                     		VARCHAR(2)		 NULL  COMMENT '예약설정 여부',
		RESERVE_DATE                   		DATETIME		 NULL  COMMENT '예약일자',
...
CREATE TABLE JNITCMSMBR(
		...
		EXPIRE_DATE							DATETIME		NULL	COMMENT '만료 일자'
) COMMENT='회원테이블';
...
/* Table Name: 샘플 테이블 */
/**********************************/
CREATE TABLE `JNIT_COM_SMPL` (
  `SMPL_ID` VARCHAR(20) NOT NULL,
  `SMPL_NM` VARCHAR(200) DEFAULT NULL,
  `CREATED_ID` VARCHAR(20) DEFAULT NULL,
  `MODIFIED_ID` VARCHAR(20) DEFAULT NULL,
  `CREATED` DATETIME DEFAULT NULL,
  `MODIFIED` DATETIME DEFAULT NULL,
  `ISDEL` VARCHAR(2) DEFAULT NULL
);

ALTER TABLE JNIT_COM_SMPL ADD CONSTRAINT IDX_JNIT_COM_SMPL_PK PRIMARY KEY (SMPL_ID);

/**********************************/
/* Table Name: 공통코드 그룹 */
/**********************************/
CREATE TABLE JNIT_COM_CODE_GRP (
	JCCG_ID			VARCHAR(20)						NOT NULL	COMMENT '그룹코드',
	JCCG_P_ID		VARCHAR(20)						NOT NULL	COMMENT '사용여부',
	JCCG_NM			VARCHAR(100)								COMMENT '사용여부',
	JCCG_LVL_VL		INT(15)										COMMENT '사용여부',
	JCCG_SORT_SEQ	INT(5)										COMMENT '사용여부',
	JCCG_CHNG_YN	VARCHAR(1)		DEFAULT 'N'					COMMENT '사용여부',
	JCCG__EXPLNT	TEXT										COMMENT '사용여부',
...
ALTER TABLE JNIT_COM_CODE_GRP ADD CONSTRAINT IDX_JNIT_COM_CODE_GRP_PK PRIMARY KEY (JCCG_ID);
CREATE INDEX JNIT_JNIT_COM_CODE_GRP_IDX1 ON JNIT_COM_CODE_GRP(JCCG_ID ASC);

/**********************************/
/* Table Name: 공통코드 */
/**********************************/
CREATE TABLE JNIT_COM_CODE (
	JCC_ID			VARCHAR(20)						NOT NULL	COMMENT '개별코드',
	JCC_P_ID		VARCHAR(20)						NOT NULL	COMMENT '상위 개별코드',
	JCCG_ID			VARCHAR(20)						NOT NULL	COMMENT '그룹코드',
	JCC_NM			VARCHAR(100)								COMMENT '코드명',
	JCC_SORT_SEQ	INT(5)										COMMENT '코드정렬순서',
	JCC_CHNG_YN		VARCHAR(1)		DEFAULT 'Y'					COMMENT '변경가능여부',
	JCC_VL1			VARCHAR(100)								COMMENT '추가코드1값',
	JCC_VL2			VARCHAR(100)								COMMENT '추가코드2값',
	JCC_VL3			VARCHAR(100)								COMMENT '추가코드3값',
	JCC_EXPLNT		TEXT										COMMENT '코드설명',
	ISDEL			VARCHAR(1)		DEFAULT 'N'		NOT NULL	COMMENT '사용여부',
	CREATED			DATETIME		DEFAULT NOW()				COMMENT '등록일시',
	CREATED_ID		VARCHAR(20)									COMMENT '등록자ID',
	MODIFIED		DATETIME		DEFAULT NOW()				COMMENT '수정일시',
	MODIFIED_ID		VARCHAR(20)									COMMENT '수정자ID'
);
ALTER TABLE JNIT_COM_CODE ADD CONSTRAINT IDX_JNIT_COM_CODE_PK PRIMARY KEY (JCC_ID, JCC_P_ID, JCCG_ID);
CREATE INDEX JNIT_COM_CODE_IDX1 ON JNIT_COM_CODE(JCC_ID ASC, JCC_P_ID ASC, JCCG_ID ASC);
ALTER TABLE JNIT_COM_CODE ADD CONSTRAINT IDX_JNIT_COM_CODE_FK0 FOREIGN KEY (JCCG_ID) REFERENCES JNIT_COM_CODE_GRP (JCCG_ID);